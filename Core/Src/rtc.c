...
/* USER CODE BEGIN 0 */
/* 备份域 magic 字：用 BKP_DR0 标记 RTC 已初始化，写 BKP_DR1 标记时间已校准 */
...
void MX_RTC_Init(void)
{
  ...
  */
  ...
  /* 备份域检查：若 RTC 从未初始化（BKP_DR0 无 MAGIC），
   * 则将时间归零并标记已初始化，后续可通过串口 SETTIME 命令校时。 */
 ...
 * 先取 Time 会锁定 Date，确保时间/日期来自同一时刻。
...
/* 写备份域寄存器 BKP_DR1，标记/清除 RTC 时间已校时标志位。 */
static void app_rtc_mark_time_valid(uint8_t is_valid)
{
  HAL_PWR_EnableBkUpAccess();
